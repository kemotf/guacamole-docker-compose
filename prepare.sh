...
mkdir ./volumes/postgres/init >/dev/null 2>&1
...
openssl req -nodes -newkey rsa:2048 -new -x509 -keyout ./volumes/nginx/ssl/self-ssl.key -out nginx/ssl/self.cert -subj '/C=DE/ST=BY/L=Hintertupfing/O=Dorfwirt/OU=Theke/CN=www.createyourown.domain/emailAddress=docker@createyourown.domain'
